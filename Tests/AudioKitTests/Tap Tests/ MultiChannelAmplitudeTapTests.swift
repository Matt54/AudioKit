...
class MultiChannelAmplitudeTapTests: XCTestCase {
    func testTapDoesntDeadlockOnStop() throws {
        let engine = AudioEngine()
        let url = Bundle.module.url(forResource: "12345", withExtension: "wav", subdirectory: "TestResources")!
        let player = AudioPlayer(url: url)!
        engine.output = player
        let tap = MultiChannelAmplitudeTap(player)
    ...
    func testDoesntCrashForMoreThenTwoChannels() {
        ...
        let tap = MultiChannelAmplitudeTap(reverb)
    ...
    func testStopResetsAllToZero() {
        ...
        tap.doHandleTapBlock(buffer: buffer, at: .now())
        tap.stop()
        for channel in 0..<tap.amplitudes.count {
            XCTAssertEqual(tap.amplitudes[channel], 1)
        }
    }
    ...
    func testDefault() {
        ...
        let tap = MultiChannelAmplitudeTap(noise) { amp in
            if abs(amp[0] - (detectedAmplitudes.last ?? 0.0)) > 0.05 {
                detectedAmplitudes.append(amp[0])
